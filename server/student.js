...
router.post('/', (request, response) => {
    const {stud_id, stud_name, stud_email, stud_prn, stud_gender, stud_bdate, stud_propic, stud_password, course_id, batch_id} = request.body
    const encryptedPassword = '' + cryptoJs.MD5(stud_password)

    const connection = db.connect()
    const statement1 = `select * from Student where stud_id = ${stud_id}`
    connection.query(statement1, (error, users) => {
        ...
        if (users.length == 0) {
            ...
            // insert a new record
            const statement = `INSERT INTO Student (stud_id, stud_name, stud_email, stud_prn, stud_gender, stud_bdate, stud_propic, stud_password, course_id, batch_id) values (${stud_id}, '${stud_name}', '${stud_email}', ${stud_prn}, '${stud_gender}', '${stud_bdate}', '${stud_propic}', '${encryptedPassword}', ${course_id}, ${batch_id})`
            connection.query(statement, (error, data) => {
                connection.end()
                response.send(utils.createResult(error, data))
            })
